...
function Navbar() {
  ...
  return (
    <nav
      ref={navbarRef}
      className={`fixed top-0 ${isHome ? "left-1/2 -translate-x-1/2 w-[90vw]  sm:w-fit" : "left-0 right-0 w-[100vw] max-w-none sm:max-w-none sm:w-full"} pl-4 pr-1.5 sm:px-4 py-3 z-50 border-b border-white/10 text-slate-50 bg-black/10 mx-auto rounded-xl min-w-[220px] ${isExpanded ? "pb-3" : ""}`}
    >
            ...
            <li
              ...
              className={`w-full sm:w-auto mb-1 sm:mb-0 transition-all duration-300 ${
                isExpanded
                  ...
                  : "max-sm:opacity-0 max-sm:transform max-sm:-translate-y-2 max-sm:invisible sm:opacity-100 sm:transform sm:translate-y-0 sm:visible"
              }`}
              style={{
            ...
            >
              <NavLink
                ...
                className={({ isActive }) => {
                  ...
                  const activeClasses =
                    "text-white bg-gradient-to-r from-yellow-400/40 to-yellow-500/40 backdrop-blur-sm border border-yellow-400/30 shadow-lg scale-105";
